...
class VoiceOutput:
    ...
    def _speak_tamil(self, text):
        ...
        try:
            ...
            threading.Timer(3.0, lambda: self._cleanup_file(filename)).start()
    ...
    def _play_audio_safe(self, filepath):
        ...
        try:
            ...
            if system == 'Windows':
                # Method 1: Use winsound (no window, built-in)
                try:
                    import winsound
                    winsound.PlaySound(filepath, winsound.SND_FILENAME | winsound.SND_NODEFAULT)
                    return
                except Exception as e1:
                    print(f"winsound failed: {e1}")
                
                # Method 2: Use PowerShell (silent, no window)
                try:
                    ps_command = f'''
